  ...
  bestsellerlist: async (req, res, next) => {
    try {
      ...
      const bestSellingProducts = await Order.aggregate([
        {
          $unwind: "$orderDetails", // Tách mảng orderDetails thành các document riêng biệt
        },
        {
          $group: {
        ...
        {
          $lookup: {
            from: "products", // Bộ sưu tập "products" chứa các documents
            localField: "_id",
            foreignField: "_id",
        ...
        {
          $unwind: "$productInfo", // Tách mảng productInfo thành các documents riêng biệt
        },
        {
          $project: {
            _id: 1,
